...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        The dictionary maps every uppercase and lowercase letter to an uppercase and lowercase letter, respectively. Vowels are shuffled according to vowels_permutation. The first letter in vowels_permutation corresponds to a, the second to e, and so on in the order a, e, i, o, u.
        The consonants remain the same. The dictionary should have 52 keys of all the uppercase letters and all the lowercase letters.
        Example: When input "eaiuo":
        Mapping is a->e, e->a, i->i, o->u, u->o and "Hello World!" maps to "Hallu Wurld!"
        Returns: a dictionary mapping a letter (string) to another letter (string).
        '''
        vowels = list(VOWELS_LOWER)
        vowels_permutation = list(vowels_permutation)
        d = dict(zip(vowels, vowels_permutation))
        return d
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for perm in permutation:
            d = self.build_transpose_dict(perm)
            text = self.apply_transpose(d)
            text = text.split()
            # print(text)
            for t in text:
                if is_word(load_words(WORDLIST_FILENAME),t):
                    count += 1
                else:
                    count += 0
        ...
        return (best_permutation[a], " ".join(result[a]))
        return text
...
if __name__ == '__main__':
    ...
    message = SubMessage("My name is Tung!")
    permutation = "ouaei"
    enc_dict = message.build_transpose_dict(permutation)
    enc_message = EncryptedSubMessage(message.apply_transpose(enc_dict))
    print("Decrypted message:", enc_message.decrypt_message())
